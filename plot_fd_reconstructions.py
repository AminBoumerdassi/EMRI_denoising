...
#Bandpass
y_pred_EMRIs_bandpassed= y_pred_EMRIs-y_true_EMRIs#signal.sosfilt(sos, y_pred_EMRIs)

#Do the FFT
f= np.fft.rfftfreq(n=no_pts, d=dt)
'''Should we be using a Tukey window first?'''
window = signal.windows.tukey(y_pred_EMRIs_bandpassed.shape[-1], alpha=1/8)
y_pred_EMRIs_bandpassed_FFT=np.abs(np.fft.rfft(window*y_pred_EMRIs_bandpassed))**2#
X_EMRIs_FFT= np.abs(np.fft.rfft(window*X_EMRIs))**2
